...
function ContactPsychiatrist() {
  ...
  // Student appointments status below header
  ...
  // Fetch student's appointments for status section
  useEffect(() => {
    const loadAppointments = async () => {
      try {
        ...
        // If not a student or no appointments, keep calm
        setApptError(err.response?.data?.error || '');
      } finally {
  ...
  return (
    <div className="appointment-booking-container">
      ...
      <div className="appointment-status-section">
        <h3 className="status-title"><i className="bi bi-calendar-check me-2"></i>Your Appointment Status</h3>
        {apptLoading ? (
          <div className="status-skeletons">
            {[...Array(2)].map((_,i)=> (
          ...
          <div className="status-empty">
            ...
            You have no appointment requests yet. Select a psychiatrist below to request one.
          </div>
        ) : (
          <div className="status-list">
            {myAppointments
              ...
              .map((appt) => {
                ...
                const timeStr = (dt.getHours() || dt.getMinutes()) ? dt.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : 'Not set';
                return (
                  <div key={appt._id} className="status-card">
                    <div className="status-main">
                      <div className="status-who">
                        <i className="bi bi-person-hearts me-2"></i>
                        <span>Dr. {appt.psychiatristId?.username || 'Psychiatrist'}</span>
                      </div>
                    ...
                    <div className="status-when">
                      <div className="when-item"><i className="bi bi-calendar3 me-1"></i>{dateStr}</div>
                      <div className="when-item"><i className="bi bi-clock me-1"></i>{timeStr}</div>
                    </div>
                  </div>
                );
              })}
...
{assessmentReport && (
                  <div className="summary-item assessment-attachment">
                    <i className="bi bi-file-earmark-medical text-success" style={{fontSize: '1.2rem'}}></i>
                    <div>
                      <strong className="badge-title">📊 ZenZone Assessment Report Attached</strong>
                      <br/>
                      <span className="badge-subtle">Wellness Score: {assessmentReport.overall_score}/100 | Risk Level: {assessmentReport.risk_level}</span>
                      <br/>
                      <small className="badge-subtle">✅ Report will be automatically shared with your psychiatrist</small>
                    </div>
                  </div>
                )}
              </div>
